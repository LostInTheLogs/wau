...
local function to_camel_case(s) return s:gsub("_(.)", string.upper):gsub("^(.)", string.upper) end

local function to_upper_case(s) return s:upper() end
...
local function convert_type_to_signature(type_name, optional)
  ...
  elseif type_name == "string" then
    return res .. "s"
  elseif type_name == "fd" then
    return res .. "h"
  elseif type_name == "fixed" then
    return res .. "f"
  elseif type_name == "array" then
    return res .. "a"
  elseif type_name == "object" then
    return res .. "o"
  elseif type_name == "new_id" then
    return res .. "n"
  elseif type_name == "uint" then
    return res .. "u"
  else
    error("Couldn't resolve type", type_name)
  end
end
...
local function get_message_types(args)
  ...
  for _, arg in ipairs(args) do
    if arg.type == "object" or arg.type == "new_id" then
      ...
      local iface = arg.interface and string.format("wau.%s", arg.interface) or "0"
...
local function get_message_signature(mes)
  ...
  for _, arg in ipairs(mes.args) do
    signature = signature .. convert_type_to_signature(arg.type, arg["allow-null"] and arg["allow-null"] == "true")
  end
...
function printer.message(mes)
  ...
  printer.line([[signature = "%s",]], get_message_signature(mes))
  printer.line([[types = %s,]], get_message_types(mes.args))
  if mes.type then printer.line([[type = "%s"]], mes.type) end
  printer.indent_sub()
  printer.line [[},]]
end
...
function printer.interface(iface)
  ...
  for _, request in ipairs(iface.requests) do
    ...
    for _, arg in ipairs(request.args) do
      if arg.type ~= "new_id" then
        printer.line(
          [[-- @tparam %s %s%s]],
          arg.type == "object" and arg.interface or arg.type,
          arg.name,
    ...
    if not returns then printer.line([[-- @treturn %s self]], iface.name) end
  ...
  for _, event in ipairs(iface.events) do
    ...
    for _, arg in ipairs(event.args) do
      printer.line(
        [[-- @tparam %s %s%s]],
        (arg.type == "object" or arg.type == "new_id") and arg.interface or arg.type,
        arg.name,
        arg.summary and " " .. arg.summary or ""
      )
    end
...
local function main(arg)
  ...
  for i, a in ipairs(arg) do
    ...
    elseif a == "-i" or a == "--indent" then
      if arg[i + 1] and tonumber(arg[i + 1]) then printer.indent_delta = tonumber(arg[i + 1]) end
    end
  end
  ...
  if printer.include_comments then io.stdout:write "-- Auto generated by the wau-scanner v0\n" end
