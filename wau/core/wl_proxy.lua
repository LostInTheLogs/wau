...
local function get_identifier_for(proxy) return proxy:get_id() end
...
local function parse_args(proxy, message, args)
  ...
  for j = 1, #signature do
    ...
    elseif c == "o" or c == "n" then
      ...
      if c == "n" then raw.wl_proxy_add_dispatcher(new_proxy, M.dispatcher, nil, nil) end
...
function M.dispatcher_func(_, proxy, _, message, args)
  ...
  for _, func in ipairs(event_callbacks) do
    ...
    if not success then io.stderr:write("Error calling callback function: ", tostring(err), "\n") end
  end
  return 0
end
...
function M:get_class() return ffi.string(raw.wl_proxy_get_class(self)) end

--- Get the ID of the proxy
-- @treturn int The ID of the proxy
function M:get_id() return raw.wl_proxy_get_id(self) end
...
function M:marshal(opcode, ...) raw.wl_proxy_marshal(self, opcode, ...) end
...
function M:marshal_constructor_versioned(opcode, iface, version, name, iface_name)
  local id = raw.wl_proxy_marshal_constructor_versioned(self, opcode, iface, version, name, iface_name, version, nil)
  return return_new_proxy(id)
end
...
function M.mt.__tostring(self) return ("<%s@%d>"):format(self:get_class(), self:get_id()) end
