...
function M.mt.__newindex(self, k, v) M.__private[ffi.string(self.name)][k] = v end

function M.mt.__tostring(self) return ("<interface:%s:v%d>"):format(ffi.string(self.name), self.version) end
